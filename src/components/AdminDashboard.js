...
const AdminDashboard = () => {
  ...
  const handleSendNotification = async () => {
    ...
    const { value } = await Swal.fire({
      ...
      html:
        '<input id="swal-input-title" class="swal2-input" placeholder="Notification Title">' +
        '<textarea id="swal-input-body" class="swal2-textarea" placeholder="Notification Body"></textarea>',
      focusConfirm: false,
      ...
      preConfirm: () => {
        return {
          ...
          body: document.getElementById("swal-input-body").value,
        };
      },
    });
    ...
    if (value && value.title && value.body) {
      try {
        ...
        Swal.fire({
          ...
          text: `Notifications sent to ${selectedUsers.length} user(s).`,
        });
        setSelectedUsers([]);
      } catch (error) {
      ...
      Swal.fire({
        icon: "error",
        title: "Invalid Input",
        text: "Both title and body are required.",
      });
    }
  };
  ...
  const handleEditTask = (taskId) => {
    ...
    setEditedTaskData({
      ...
      penalty: globalTasks[taskId]?.penalty || 0,
    ...
    try {
      ...
      if (
        ...
        Swal.fire({
          ...
          text: "Numeric values cannot be negative.",
        });
        return;
      }
      ...
      const updates = {
        ...
        frequencyUnit: editedTaskData.frequencyUnit || "days",
  ...
  const handleSaveMessageEdit = async (userId) => {
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Update Failed",
        text: "Failed to update admin message.",
      });
    }
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        ...
        <div>
          ...
          <button style={styles.bulkButton} onClick={handleSendNotification}>
            Send Notification
          </button>
        </div>
